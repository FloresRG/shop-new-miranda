...
export default function ProductDetail({ product }: ProductDetailProps) {
    ...
    const [mainImage, setMainImage] = useState(product.fotos[0]?.foto || defaultImage);
    ...
    return (
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12 animate-in slide-in-from-bottom duration-500">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2">
                        {product.fotos.map((f, i) => (
                            <button
                                ...
                                className={`w-20 h-20 flex-shrink-0 rounded-lg overflow-hidden border-2 transition-all ${mainImage === f.foto ? 'border-primary opacity-100' : 'border-transparent opacity-70 hover:opacity-100'}`}
                            >
                                <img src={f.foto} className="w-full h-full object-cover" />
                            </button>
                        ))}
                    </div>
                )}
            ...
            <div className="flex flex-col h-full">
                <div className="mb-4">
                    <span className="text-primary font-semibold uppercase tracking-wider text-sm">{product.categoria.categoria} / {product.marca.marca}</span>
                    <h1 className="text-3xl lg:text-4xl font-bold mt-2 text-dark dark:text-white mb-2">{product.nombre}</h1>
                    {stock > 0 && stock <= 5 && (
