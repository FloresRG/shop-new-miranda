...
export default function ContactForm() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.celular.trim()) errors.celular = "Por favor, ingrese su número de celular.";
    ...
    if (productos.length === 0)
      errors.productos = "Por favor, suba al menos una imagen de los productos.";
    if (!comprobanteFile) errors.comprobante = "Por favor, suba el comprobante de pago.";
  ...
  const sendContactToAPI = async () => {
    try {
      ...
      // Reset
      setFormData({ nombre: "", ci: "", celular: "", departamento: "", provincia: "" });
      setProductos([]);
    ...
    <div className="bg-white dark:bg-darkmode-light rounded-2xl shadow-2xl p-4 border border-gray-100 dark:border-darkmode-border">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          {productos.length === 0 ? (
            <div className="cursor-pointer" onClick={() => productosFileInputRef.current?.click()}>
              <div className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 dark:border-darkmode-border rounded-xl p-10 text-center hover:border-primary hover:bg-primary/5 transition-colors">
                ...
                <p className="text-base font-medium text-gray-700 dark:text-gray-300">Toca para subir</p>
              </div>
            </div>
          ) : (
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (val === "" || (!isNaN(num) && num >= 0)) {
                            setProductos((prev) => {
                              const newProductos = [...prev];
                              newProductos[index].quantity = val === "" ? 0 : num;
                              return newProductos;
                            });
                          }
                        }}
          ...
          {comprobanteFile ? (
            ...
            <div className="relative">
              ...
              <div className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 dark:border-darkmode-border rounded-xl p-6 text-center bg-gray-50 dark:bg-darkmode-body transition-colors hover:border-primary">
                ...
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Arrastre aquí o haga clic</p>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Solo una imagen (PNG, JPG)</p>
              </div>
            </div>
          )}
      ...
      {isSubmitting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-darkmode-light rounded-2xl shadow-2xl p-8 flex flex-col items-center max-w-sm w-full mx-4">
            ...
            <p className="text-gray-800 dark:text-white font-semibold text-lg">Procesando pedido...</p>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-darkmode-light rounded-2xl shadow-2xl p-8 flex flex-col items-center max-w-sm w-full mx-4 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-6">
              Su pedido <span className="font-bold text-primary">#{contactId}</span> ha sido registrado.
            </p>
            <button
