        ...
        const fetchData = async () => {
            try {
                ...
                const itemPromises = topCategories.map(async (cat) => {
                    try {
                        const resProd = await fetch(`/api/productos?categoria_id=${cat.id}&page=1`);
                        if (resProd.ok) {
            ...
            <section className="section pt-0">
                <div className="container">
                    ...
                    <div className="flex gap-4 overflow-hidden">
                        {[1, 2, 3, 4].map(i => (
                            <div key={i} className="min-w-[280px] h-[350px] bg-gray-200 dark:bg-gray-700 animate-pulse rounded-2xl" />
                        ))}
                    </div>
                </div>
            </section>
                                ...
                                <img
                                    src={
                                        product.fotos[0]?.foto
                                            ? `https://test.importadoramiranda.com/storage/${product.fotos[0].foto}`
