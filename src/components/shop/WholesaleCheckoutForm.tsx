...
export default function WholesaleCheckoutForm() {
  ...
  const calculateItemPrice = (item: any): number => {
    ...
    if (item.isWholesale) {
      if (q >= 12 && item.precio_docena)
        return parseFloat(item.precio_docena) || 0;
  ...
  const sendOrderToAPI = async () => {
    try {
      ...
      // WhatsApp
      const mensaje = `Hola, me pongo en contacto para informarles que mi pedido MAYORISTA es el número: #${pedidoNumero}.\n\nAgradezco su atención y quedo atento(a) a su confirmación respecto a este pedido.`;
      window.open(
      ...
      setIsSubmitting(false);
      toast.error(
        "Hubo un error al procesar su pedido. Por favor intente nuevamente.",
      );
    }
  };

  return (
    <div className="flex flex-col lg:flex-row gap-6 lg:gap-10 w-full max-w-full overflow-hidden bg-[#0c0c0c] text-white">
      {/* Resumen de Orden */}
      <div className="w-full lg:w-5/12 order-1 lg:order-2 flex flex-col">
        <div className="bg-[#1a1a1a] rounded-2xl shadow-xl border border-[#D4AF37]/20 overflow-hidden">
              ...
              <div key={item.id} className="flex gap-4 items-center group">
                <div className="relative w-20 h-20 shrink-0">
                  <img
                    ...
                    className="w-full h-full object-cover rounded-xl bg-[#0c0c0c] border border-[#D4AF37]/20 group-hover:scale-105 transition-transform duration-300"
                    alt={item.nombre}
                  />
                </div>

                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-white line-clamp-2 leading-tight mb-2">
                    {item.nombre}
                  </p>

                  <div className="flex items-center justify-between">
                    {/* Controles de Cantidad */}
                    <div className="flex items-center gap-3 bg-[#0c0c0c] rounded-lg p-1 border border-[#D4AF37]/20">
                      <button
                        type="button"
                        onClick={() =>
                          updateQuantity(item.id, item.quantity - 1)
                        }
                        className="w-6 h-6 flex items-center justify-center bg-[#1a1a1a] rounded-md shadow-sm text-[10px] hover:text-[#D4AF37] transition-colors border border-[#D4AF37]/20 text-white"
                      >
                        <FaMinus />
                      </button>
      ...
      <form
        ...
        className="w-full lg:w-7/12 order-2 lg:order-1 flex flex-col space-y-6"
      >
        <div className="bg-[#1a1a1a] rounded-2xl shadow-xl border border-[#D4AF37]/20 p-5 md:p-8">
          <div className="text-center mb-8 border-b border-[#D4AF37]/10 pb-6">
            ...
            <p className="text-gray-400 mt-2 text-sm">
              Complete sus datos para procesar su orden mayorista
            </p>
          </div>
              ...
              {formData.departamento && (
                <div className="flex-1 w-full">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {(
                      departamentos[
                        formData.departamento as keyof typeof departamentos
                      ] || []
                    ).map((prov) => (
                      <button
                        key={prov}
                        ...
                        className={`flex-1 min-w-[140px] px-4 py-3 border rounded-xl text-[10px] font-black transition-all transform hover:scale-[1.03] active:scale-95 flex items-center justify-center text-center leading-tight ${
                          formData.provincia === prov
                            ? "bg-gradient-to-r from-[#C5A021] to-[#D4AF37] text-white border-transparent shadow-lg"
      ...
      {isSubmitting && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
          <div className="bg-[#1a1a1a] rounded-3xl shadow-2xl p-10 flex flex-col items-center max-w-sm w-full mx-4 border border-[#D4AF37]/20">
            <div className="w-16 h-16 border-4 border-t-transparent border-[#D4AF37] rounded-full animate-spin mb-6"></div>
      ...
      {isSuccess && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
          <div className="bg-[#1a1a1a] rounded-3xl shadow-2xl p-10 flex flex-col items-center max-w-md w-full mx-4 text-center border border-[#D4AF37]/20">
            <div className="w-20 h-20 bg-[#D4AF37]/20 rounded-full flex items-center justify-center mb-6 text-[#D4AF37] animate-bounce">
            ...
            <div className="space-y-4 mb-8">
              <p className="text-gray-400 text-sm leading-relaxed">
                Su pedido ha sido enviado con éxito a nuestro centro de
                logística.
              </p>
              <div className="bg-[#0c0c0c] p-4 rounded-2xl border border-[#D4AF37]/20">
                <p className="text-xs font-bold text-gray-300">
                  <span className="text-[#D4AF37]">IMPORTANTE:</span> Nuestro
                  encargado de pedidos verificará físicamente si todos tus
                  productos están disponibles.
                  <strong>
                    {" "}
                    Por favor, espera nuestra confirmación antes de realizar el
                    pago.
                  </strong>
                </p>
              </div>
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-[#C5A021] to-[#D4AF37] text-white py-4 rounded-xl font-black text-lg shadow-lg hover:shadow-xl transition-all uppercase tracking-widest"
            >
              Entendido
            </button>
          </div>
